...
bool passesPtAndEta = jet->pt() > 30 && fabs(jet->eta()) < 2.5; //jet->pt() > 30 && fabs(jet->eta()) < 2.5
...
bool TopPairMuPlusJetsReferenceSelection::isGoodPhoton(const PhotonPointer photon, const EventPtr event) const {
...
const PhotonCollection TopPairMuPlusJetsReferenceSelection::signalPhotons(const EventPtr event) const {
